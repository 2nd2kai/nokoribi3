...
function pickBestLoc(game){
  ...
  if(MVP_MODE){var asf=getActiveSentFire(game);if(asf)return"unexplored_forest";}
  var sc={};
...
function applyAction(s,events){
  ...
  /* MVP_MODE: sentFireが森にあるあいだはトイマンは常に探索中。シミュレーションで"resting"に変わらない */
  if(MVP_MODE&&hasExploring){t.lastAction="exploring";t.location="unexplored_forest";}
  if(isCharMet(s,"kana")){
...
const SHADOW_VOICES={
  "未受領の影":["どうせ届いてない","見られていないものは、なかったのと同じだ","その火、誰が受け取るの？","ちゃんと届いた？","本当に伝わったの？"],
  "足あと喰い":["誰も覚えていない","歩いた跡なんて、すぐ消える","残したつもり？","それ、意味あった？","もう誰の記憶にもない"],
  "迷子の声":["なんで書いたの？","どこに届いてほしかったの？","まだ名前もないくせに","自分でも分かってないんじゃない？","誰のために書いたの？"],
  "錆びた判決札":["価値なし","通過不可","これは保留ではなく却下だ","最初から無駄だった","誰もそれを求めていなかった"],
  "何にもならなかった獣":["ほら、何にもならなかった","時間だけが燃えた","誰にも必要とされていない","あなたがやらなくても同じだった","それで何が変わった？"],
  "どうせ届かない鳥":["飛ばしても返事は来ない","空に投げても落ちるだけ","宛先なんてない","届いたとしても、何も変わらなかった","誰が読んでくれると思ってたの？"]
};
...
function getPersonalShadowVoices(card){
  var feeling=card&&card.feeling||"";
  var ws=card&&card.writeState||"";
  var wanted=card&&card.wanted||"";
  var voices=[];
  var wsV={
    ...
    "投稿したけど反応がない":[
      ...
      "反応がなくても、それを出したあなたは何を伝えたかった？",
      "「いいね」ではなく、本当は何が欲しかった？",
      "出した瞬間の気持ち、今でも覚えてる？",
      "もし反応が来ていたら、何が変わってたと思う？",
      "沈黙の中に、あなたはどんな声を聞いてた？"
    ],
    "終わったのに虚しい":[
      ...
      "書き終えた瞬間、何を感じた？",
      "虚しさは、ゴールにたどり着けなかったから？それとも、ゴール自体が違ったから？",
      "この虚しさ、前にも感じたことある？",
      "終わりと思ってたものが、まだ続いてるとしたら？",
      "何が起きたら、虚しくなかったと思う？"
    ],
    "何にもならない気がした":[
      ...
      "書く前の自分に、今のあなたは何と言う？",
      "「何かになる」って、どういうことだと思ってた？",
      "書いたことで、何かが変わった、という微細なサインを見逃してない？",
      "あなたが「何かにならない」と思うとき、誰の声が聞こえてる？",
      "それを書いた自分を、誰かに見せたかった？"
    ],
    "誰にも見せていない":[
      "見せていないのに、なぜまだ持っているの？",
      ...
      "これを書いたとき、誰かの顔が浮かんでた？",
      "いつか誰かに見せる日が来ると思ってる？",
      "見せていないのは、誰かを守るため？自分を守るため？",
      "誰にも見せなかった理由、一番正直に言うと？",
      "もし見せていたら、何が起きると思ってた？",
      "見せなかったことを、後悔してる？それとも正解だった？"
    ],
    "消したいけど消せない":[
      "消せないでいる理由、自分でも分かる？",
      "消せないのは、それがまだ大事だから？",
      ...
      "消したいと思ったのは、いつから？",
      "消したいのは、誰かに見られたくないから？それとも自分が見たくないから？",
      "消せないままにしていることで、何かを守ってる気がする？",
      "もし消せたとして、その後どんな気持ちになると思う？",
      "消えないのは、あなたが手放せていないから。それは何を意味してる？"
    ]
  };
  ...
  var feV={
    ...
    "空っぽ":[
      ...
      "空っぽでいる自分を、誰かに見せたかった？",
      "空っぽの中に、何かまだ残ってるものはある？",
      "これだけ空っぽになるまで注いだ理由、何だと思う？",
      "誰かに「お疲れ様」と言われたら、どう感じると思う？"
    ],
    "悲しさ":[
      "その悲しさ、誰かに分かってほしかった？",
      ...
      "この悲しさ、いつ頃から始まってた？",
      "悲しみながらも書いたのは、なぜ？",
      "もし悲しさが声を持ったら、最初に何を言うと思う？",
      "悲しさを一番理解してほしかった人、誰だった？"
    ],
    "恥ずかしさ":[
      "恥ずかしいと思いながら、なぜ出したの？",
      ...
      "恥ずかしさは、何が人に知られるのが怖かったから？",
      "恥ずかしさを乗り越えて出した。その勇気に気づいてる？",
      "恥ずかしくなかったとしたら、もっと伝えたかったことがある？"
    ],
    "怒り":[
      "その怒り、何に向かっていた？",
      "怒りを言葉にすることで、少し楽になった？",
    ...
    "寂しさ":[
      ...
      "寂しさを誰かに見せることができなかった理由は？",
      "寂しさが一番強かった瞬間、どんな状況だった？",
      "その寂しさ、今でもまだそこにある？",
      "寂しさをそのまま言葉にできたら、どんな言葉になる？"
    ],
    "情けなさ":[
      "情けないと感じたのは、自分にどんな期待をしていたから？",
      "情けなさを感じられるということは、何かを大事にしてた証じゃない？",
      "もし自分に優しくするとしたら、何と言う？",
      "情けなさの奥に、本当はどうなりたかった気持ちがある？",
      "その情けなさ、誰かに見せてもいいと思える？",
      "情けなくても書いた。それはどういうことだと思う？"
    ],
    "疲れ":[
      "いつから疲れてた？",
      "疲れていても書いた。何がそうさせた？",
      "その疲れ、誰かに気づいてほしかった？",
      "疲れているのに手を動かした自分を、どう思う？",
      "今、一番休めているものは何？",
      "疲れが取れたとして、一番最初に何をしたい？"
    ],
    "まだ分からない":[
      ...
      "分からないのは、まだ近すぎるから？",
      "いつか分かるようになると思う？それとも、分からないままでいい？",
      "この「分からなさ」、前にも感じたことある？"
    ]
  };
  if(feV[feeling])voices=voices.concat(feV[feeling]);
  if(wanted&&wanted!=="まだ分からない")voices.push("「"+wanted+"」を求めていた。それを一番分かってほしかったのは、誰？");
  if(wanted&&wanted!=="まだ分からない")voices.push("「"+wanted+"」が欲しかった。そこに何があれば、満たされてたと思う？");
  if(voices.length===0)voices=["この火を預けたとき、何を期待してた？","書いた後、何が変わると思ってた？","もし誰かが受け取ったとしたら、その人に何を伝えたかった？","それを書いた自分のこと、どう思ってる？","書かなかったとしたら、どうなってたと思う？"];
  return voices;
}
...
const SOFTENED_SHADOW_VOICES=[
  ...
  "……聞こえては、いる。",
  "……そうか。そう思って、書いたのか。",
  "……何度も向き合ってくれた。もう少し、一緒にいてもいい？",
  "……最初は鋭くいたけど、今はただ、そこにある。",
  "……ずっと消えるものだと思ってた。でも消えなかった。",
  "……あなたが言葉を返すたびに、少し小さくなっていく。",
  "……もう叫ばなくていい気がしてきた。",
  "……まだいるよ。でも、もう攻めない。",
  "……あなたの言葉が、ここにも届いてた。"
];
...
function makeBattlePreview(game){
  ...
  return{enemy:enc.enemy,hp:hp,hpMax:hpMax,fatigue:fatigue,progress:pct,mod:mod,watch:Math.round(b.watch||0),cooldownMs:left,turns:enc.turns||0,maxTurns:enc.maxTurns||BATTLE_CFG.encounterMaxTurns,voice:getShadowVoiceForCard(card,enc.voiceIndex||0),card:card};
}
...
function runToymanBattleIntervention(game,actionId,selfAnswer){
  ...
  var mvpBattleOk=MVP_MODE&&card&&(ns.sentFires||[]).some(function(f){return isWorldFire(f)&&f.dest==="forest"&&f.companion==="toyman";});
  if(!t||!card||(!mvpBattleOk&&t.lastAction!=="exploring")){return{ok:false,state:game,msg:"向き合える状態ではありません",lines:["トイマンが未受領の森で残り火を回収中のときだけ、影と向き合えます。"]};}
  if(mvpBattleOk&&t.lastAction!=="exploring"){t.lastAction="exploring";t.location="unexplored_forest";}
  var nowT=Date.now();
  ...
  var lines=["影と向き合った — "+action.label,"影の声：「"+voice+"」"];
  if(action.kind==="hold"){
  ...
  if(selfAnswer)lines.push("あなたの言葉：「"+selfAnswer+"」— この言葉は残り火に刻まれた");
  var ended=false;
  ...
  if(events.length>0){ended=true;events.forEach(function(e){lines.push(e.text);});ns.battle.encounter=null;}
    ...
    lines.push(action.kind==="retreat"?"トイマンは今日の分を終えた。またいつでも来られる。":"影と向き合った。道が少し開いた。");
    ns.ip.cur=Math.min(ns.ip.max,(ns.ip.cur||0)+1);
    lines.push("向き合いを越えた：干渉ポイント +1");
  ...
  return(
    <div className="battle-screen scroll">
      <div className="battle-full-hero">
        ...
        <p>残り火を取り戻すトイマンの道に、影の声が現れた。<br/>あなたが書いたものへの問い。向き合うと、言葉が残り火に刻まれます。</p>
      </div>
      {card&&<div className="battle-ember-full">
        <div className="battle-ember-meta">
          {card.writeState&&<span>{card.writeState}</span>}
          {card.feeling&&<span>{card.feeling}</span>}
          {card.wanted&&<span>本当は：{card.wanted}</span>}
        </div>
      ...
      {result&&<div className={"battle-result"+(result.ok?" battle-result-ok":" battle-result-ng")+(result.reached100?" battle-result-reached":"")}>
        ...
        <div className="battle-result-toyman">
          <div className="home-cv-namerow"><span className="isc-dot cd-toyman"/><span className="home-cv-name">トイマン</span></div>
...
function NowSceneView(p){
  ...
  var mvpSFBattle=MVP_MODE&&(game.sentFires||[]).some(function(f){return isWorldFire(f)&&f.dest==="forest"&&f.companion==="toyman"&&!f.questionPending&&(f.progress||0)<100;});
  var canBattle=loc==="unexplored_forest"&&battleCard&&(game.characters.toyman.lastAction==="exploring"||mvpSFBattle);
  ...
  return(
    <div className="now-wrap">
      ...
      {!MVP_MODE&&<div className="now-action-bar">
        {(function(){
      ...
      {/* 次に起きそうな変化 */}
      {!MVP_MODE&&upcomingChanges.length>0&&<div className="now-upcoming">
      ...
      {/* 工程 */}
      {!MVP_MODE&&<div className="now-stage-panel">
      ...
      {!MVP_MODE&&<div className="now-prog">
      ...
      {!MVP_MODE&&pressureOpen&&((function(){
...
function HomeView(p){
  ...
  return <div className="scroll home-screen">
    ...
    {/* 4. 灯貨ミニ */}
    {!MVP_MODE&&(game.toka&&game.toka.total>0)&&<div className="home-toka-mini">
    ...
    {/* 5. 箱庭の奥 */}
    {!MVP_MODE&&<section className="home-oku">
...
/* ══════════════════════════════════════════════════════════════
   火を送り出す ── B-loop 第一スライス
   預け → 送り先×同行者 → 掘る/置く問い → 答え → 受領証/置き札/預かり札 → 棚
   （既存 emberCards パイプラインとは独立。データは game.sentFires）
   ══════════════════════════════════════════════════════════════ */
const J_DESTS=[
...
function jReturnReport(comp,placeMode,retreated){
  var n=jName(comp);
  if(retreated)return n+"：「途中まで行った。今夜は、ここまでで持ち帰った。」";
  if(placeMode)return n+"：「奥までは行かずに、戻ってきた。」";
  return ({toyman:n+"：「奥まで行った。これを見つけた。」",kana:n+"：「そばにいた。これを持って帰ってきた。」",utsuro:n+"：「拾って、封筒に入れて戻った。」"})[comp];
}
function jReply(comp,deferred,placeMode){
  if(deferred)return ({toyman:"いい。まだで。預かっとく。",kana:"いいよ、まだで。そばに置いとく。",utsuro:"終わらせなくていい。置いておく。"})[comp];
  if(placeMode)return ({toyman:"うん。今夜は、ここに置いた。",kana:"名前にしなくていい。置いておこう。",utsuro:"終わってない。それで、いい。"})[comp];
...
function EmberJourney(p){
  ...
  var destObj=J_DESTS.find(function(d){return d.id===dest;});
  ...
  return <div className="ov ej-ov" onClick={p.onClose}><div className="bsh ej-bsh" onClick={function(e){e.stopPropagation();}}>
    ...
    {phase==="deposit"&&<div className="ej-in">
      ...
      <label className={"ej-field"+(noWords?" ej-dim":"")}><span>その奥にあるもの（任意）</span>
      ...
      {!MVP_MODE&&<button className={"ej-chip ej-chip-soft"+(tooHard?" ej-chip-on":"")} onClick={function(){setTooHard(function(v){return !v;});}}>今日は問いにしないで、預かってほしい</button>}
    ...
    {phase==="send"&&<div className="ej-in">
      <p className="ej-step">送り先 ＝ どんな問いに会うか</p>
      <div className="ej-cards">{J_DESTS.map(function(d){return <button key={d.id} className={"ej-dest"+(dest===d.id?" ej-sel":"")} onClick={function(){setDest(d.id);}}>
        <span className="ej-dest-label">{d.label}</span><span className="ej-dest-verb">{d.verb}</span><span className="ej-dest-desc">{d.desc}</span>
      </button>;})}</div>
    ...
    {phase==="travel"&&<div className="ej-in ej-travel">
      <p className="ej-travel-line">{jName(comp)}が、{destObj&&destObj.label}へ向かった。</p>
      <p className="ej-travel-sub">あなたが離れているあいだに、火は旅をする。</p>
      <button className="btn btn-p ej-go" onClick={function(){returnFire(false);}}>このまま、奥まで探してもらう</button>
      <button className="btn btn-g" onClick={function(){returnFire(true);}}>今夜は、ここまで探してもらう</button>
    </div>}
    ...
    {phase==="kept"&&result&&<div className="ej-in ej-kept">
      ...
      <div className="ej-formed jf-kept">
        ...
        <span className="ej-formed-title">「{jTitleOf(result)}」</span>
        <span className="ej-formed-note">この火は、答えを求めずに預かった。</span>
      </div>
      <button className="btn btn-g ej-go" onClick={p.onClose}>閉じる</button>
    </div>}
...
function App(){
  ...
  useEffect(function(){
    ...
    var et=setInterval(function(){var g=gameRef.current;if(!g)return;if(MVP_MODE){var asf=(g.sentFires||[]).find(function(f){return isWorldFire(f)&&f.dest==="forest"&&f.companion==="toyman";});if(!asf)return;var toyLines=["トイマンは、森の奥へ進んでいる。","トイマン：「まだある。もう少しだ。」","静かな森に、残り火が揺れている。","トイマン：「火は、ここにある。」"];var liveText=toyLines[Math.floor(rnd()*toyLines.length)];setLive(function(prev){return [{text:liveText,kind:"explore_detail",time:nowISO()}].concat(prev).slice(0,40);});setGame(function(prev){if(!prev)return prev;var ns=cloneS(prev);var sf=(ns.sentFires||[]).find(function(f){return isWorldFire(f)&&f.dest==="forest"&&f.companion==="toyman";});if(!sf)return ns;sf.progress=Math.min(100,(sf.progress||0)+1);if(sf.progress>=100&&!sf.questionPending){sf.questionPending=true;sf.foundBy="toyman";sf.foundAt=nowISO();sf.foundQuestion=makeSentFireQuestion(sf);unlockPlace(ns,"record_tower",false);appendEventLog(ns,"トイマンが「"+jTitleOf(sf)+"」の問いの欠片を見つけた","discover");}return ns;});return;}var ev=genLiveEvent(g);setLive(function(prev){return [{text:ev.text,kind:ev.kind,time:nowISO()}].concat(prev).slice(0,40);});setGame(function(prev){if(!prev)return prev;var ns=ev.su?applyLU(prev,ev.su):cloneS(prev);maybeSpawnToymanDiscoveryLetter(ns);tickLettersAmbient(ns);return ns;});},22000);
  ...
  return(<div className="root"><div className="frame">
    ...
    {screen!=="closed"&&screen!=="ending"&&<>
      ...
      {screen!=="battle"&&(function(){
        var logStories=getUnlockedStories(game);var logReads=game.readStories||[];
        var hasUnreadLog=logStories.some(function(s){return logReads.indexOf(s.id)===-1;});
        var unlockedConvs=getUnlockedConvIds(game);var readConvs=game.readConvs||[];
        var hasUnreadConv=unlockedConvs.some(function(id){return readConvs.indexOf(id)===-1;});
        return <nav className="bnav">
          {getVisibleTabs(game).map(function(t){
            var isNext=nextAction&&nextAction.screen===t.id&&screen!==t.id;
            var isNew=(t.id==="log"&&hasUnreadLog)||(t.id==="conv"&&hasUnreadConv);
            return <button key={t.id} className={"nbtn"+(screen===t.id?" nbtn-on":"")+(isNext?" nbtn-next":"")+(isNew&&screen!==t.id?" nbtn-new":"")} onClick={function(){if(t.id!=="conv")setViewConv(null);setScreen(t.id);}}>
              {t.label}{(isNext||isNew)&&screen!==t.id&&<span className="nbtn-dot"/>}
            </button>;
          })}
        </nav>;
      })()}
...
function EmberView(p){
  var game=p.game,onReceive=p.onReceive,onOpenCreate=p.onOpenCreate,onDepart=p.onDepart,onDelete=p.onDelete,onAdvance=p.onAdvance,onBurnReceipt=p.onBurnReceipt,onEditEmber=p.onEditEmber,onOpenBattle=p.onOpenBattle;
  var next=getNextAction(game);
  ...
  var [legacyOpen,setLegacyOpen]=useState(!MVP_MODE&&legacyNeedsAction.length>0);
  useEffect(function(){if(!MVP_MODE&&legacyNeedsAction.length>0)setLegacyOpen(true);},[legacyNeedsAction.length]);
      ...
      {card.unitState==="exploring"&&(function(){
        ...
        return <div className="ev-exploring-box">
          ...
          <div className="eeb-face-row">
            ...
            <span className="eeb-watch-hint">向き合うと、あなたの言葉が残り火に刻まれます</span>
          </div>
        </div>;
      })()}
...
function ReceiptCard(p){
  ...
  return <div className={"ev-receipt"+(grad?" ev-receipt-grad":"")}>
    ...
    {(r.shadowAnswers||[]).length>0&&<div className="ev-shadow-answers">
      <div className="bsa-log-title">向き合いの記録（{r.shadowAnswers.length}回）</div>
      {r.shadowAnswers.map(function(a,i){return <div key={i} className="bsa-log-item">
        <span className="bsa-shadow">「{a.shadow}」</span>
        {a.answer&&<span className="bsa-ans">→「{a.answer}」</span>}
        {!a.answer&&<span className="bsa-ans bsa-ans-silent">— 言葉なしで前に進んだ</span>}
      </div>;})}
    </div>}
